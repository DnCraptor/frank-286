/**
 * frank-286 - i286 PC Emulator for RP2350
 *
...
#ifndef FRANK_286_VERSION_MAJOR
#define FRANK_286_VERSION_MAJOR 1
#endif
#ifndef FRANK_286_VERSION_MINOR
#define FRANK_286_VERSION_MINOR 0
#endif
...
static int load_config_from_sd(const char *filename) {
    ...
    // Debug: List 286 directory contents
    DBG_PRINT("Checking SD card contents...\n");
    res = f_opendir(&dir, "286");
    if (res == FR_OK) {
        DBG_PRINT("  286/ directory found, contents:\n");
        ...
        DBG_PRINT("  286/ directory not found (error %d)\n", res);
        // Try root directory
        res = f_opendir(&dir, "");
        if (res == FR_OK) {
    ...
    char path[256];
    snprintf(path, sizeof(path), "286/%s", filename);
    ...
    // Also parse frank-286-specific settings
    ini_parse_string(content, parse_frank_286_ini, NULL);
...
static bool init_hardware(void) {
    ...
    // Check if 286/ directory exists
    DIR dir;
    res = f_opendir(&dir, "286");
    if (res != FR_OK) {
        show_error_screen(" Missing Directory ", "Directory '286/' not found on SD card.", "Create it and add config.ini, bios.bin");
        // show_error_screen never returns
    }
    f_closedir(&dir);
    DBG_PRINT("  286/ directory found\n");

    // Load frank-286-specific hardware settings from INI
    // This allows cpu_freq and psram_freq to be configured
    {
        FIL fp;
        char *content = NULL;

        if (f_open(&fp, "286/config.ini", FA_READ) == FR_OK) {
            FSIZE_t size = f_size(&fp);
            ...
            if (content) {
                ...
                if (f_read(&fp, content, size, &bytes_read) == FR_OK) {
                    ...
                    // Parse just the [frank-286] section
                    ini_parse_string(content, parse_frank_286_ini, NULL);
                }
                free(content);
            }
...
static bool init_emulator(void) {
    ...
    pc = pc_new(platform_poll, NULL, NULL, &config);
    if (!pc) {
    ...
    // (these override INI values if not present in [frank-286] section)
    config_set_mem_size_mb(config.mem_size / (1024 * 1024));
    config_set_fpu(config.fpu);
    config_set_redirector(config.redirector);
    // Hardware settings are loaded from [frank-286] section via parse_frank_286_ini
...
int main(void) {
    ...
    DBG_PRINT("\n\n");
    DBG_PRINT("============================================\n");
    DBG_PRINT("  frank-286 - 286 Emulator for RP2350\n");
